...
class Row:
    ...
    def set_total(self):
        ...
        """
        self.total = round(self.product.unit_price * self.qty, 2)
    ...
    def update_product_qty(self, product: Product, qty: float):
        ...
        for row in self.rows:
            if row.product == product:
                row.increase_qty_by(qty)
                break
        else:
            self.add_row(Row(product, qty))
...
def read_file(filename: str):
    ...
    with open(filename) as file:
        ...
        for item in content_list:
            ...
            for i in res:
                if i.invoice_no == invoice_no:
                    i.update_product_qty(product, qty)
                    break
            else:
                row = Row(product, qty)
